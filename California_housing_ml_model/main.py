...
if not os.path.exists(MODEL_FILE) :
    ...
    tranformed_input = pipeline.transform(input_features)
    predictions = model.predict(tranformed_input)
    output_data = pd.concat([input_data, pd.Series(predictions, name = "predicted_median_house_values")], axis = 1)

    output_data.to_csv("output.csv", index = False)
    print("Inference complete, result saved to the output.csv")
